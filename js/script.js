        ...
        reader.onload = function(e) {
          ...
          $(".export-as-lin-app").on("click", function() {
            JSZipUtils.getBinaryContent("YourLinApp.zip", function(err, data) {
              ...
              zip.file("README.md", "### Instructions\n 1. Extract the `"+ $(".appname").val().replace(/ /g, "-") +"-lin.zip` folder anywhere on your computer except the home folder. \n 2. Open a terminal and then navigate to "+ $(".appname").val().replace(/ /g, "-") +"'s directory and `run the make.sh file`.\n\n  **example**:\n  cd Downloads/"+ $(".appname").val().replace(/ /g, "-") +"-lin\n\n 3. This will move the "+ $(".appname").val().replace(/ /g, "-") +" sibling folder and it's decendants to your home directory and create an application launcher.\n");
          ...
          $(".export-as-chrome-app").on("click", function() {
            ...
            var content = zip.generate({type:"blob"});
            saveAs(content, $(".zipname").val() + "-chrome.zip");
            return false;
          });
          return false;
        };
